...
def get_output_path(request, path_format, path_format_var):
    ...
    if not OutputPath:
        #从前端传照片进来，没有路径，只有文件，这里判定路径，如果没有就取config里面的的
        OutputPath = USettings.UEditorUploadSettings[path_format]
        # 这一步是构造保存的url需要用到的参数，这样构造了之后，上传了就可以在前端页面显示，直接使用这个地址访问后台
        OutputPathFormat = OutputPath + '/' + OutputFile
